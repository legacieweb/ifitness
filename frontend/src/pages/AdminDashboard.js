...
export default function AdminDashboard() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [statsRes, usersRes, bootcampsRes] = await Promise.all([
        fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/admin/stats`, { headers }),
        fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/admin/users`, { headers }),
        fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/bootcamps`, { headers }),
      ]);
  ...
  const handleBootcampSubmit = async (e) => {
    ...
    try {
      ...
      const url = bootcampEditingId
        ? `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/bootcamps/${bootcampEditingId}`
        : `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/bootcamps`;
  ...
  const handleBootcampDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this bootcamp?')) {
      try {
        const token = localStorage.getItem('token');
        await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/bootcamps/${id}`, {
          method: 'DELETE',
