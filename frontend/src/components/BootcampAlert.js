...
export default function BootcampAlert() {
  ...
  const fetchBootcampInfo = async () => {
    try {
      ...
      const upcomingRes = await fetch('http://localhost:5000/api/bootcamps/upcoming');
      const activeRes = await fetch('http://localhost:5000/api/bootcamps/active');
  ...
  const handleAcceptBootcamp = async (bootcampId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/bootcamps/${bootcampId}/accept`, {
        method: 'POST',
